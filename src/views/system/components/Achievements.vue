                    ...
                    <ul class="content-head border-top-none" v-for="(item1,key1) in item.children" :key="key1">
                        ...
                        <li class="li-three">
                            <el-form-item :prop="'data.'+key+'.children.'+key1+'.value'" :rules="[
                                { required: true,validator: litigationMoney, trigger: 'blur' }
                            ]">
                                <el-input v-model="item1.value" placeholder="请填写全面营销人员分配比例" size="small"></el-input>
                            </el-form-item>
                        </li>
                    </ul>
                </div>
...
export default {
    ...
    methods: {
        async initData(obj,configType,flag) {
            this.dialogForm.data = attributeJX
            ...
            if(obj.configFlag && flag === 'UPDATE') { 
                let res = await this.ProductConfig.getConfigDetails(obj.id,'JX')
                if (res.code === "0000") {
                    ...
                    this.dialogForm.data.forEach(item=>{
                        item.children.forEach(item1=>{
                            if(item1.key == 'qmyxRatio') {
                                item1.value = (content[item1.key]*100).toFixed(2)
                            } else {
            ...
            this.$refs["dialogForm"].validate(async (valid) => {
                if (valid) {
                    ...
                    this.dialogForm.data.forEach(item=>{
                        item.children.forEach(item1=>{
                            if(item1.key == 'qmyxRatio') {
                                content[item1.key] = (item1.value/100).toFixed(4)
                            } else {
