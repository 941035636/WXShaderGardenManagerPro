  ...
  >
    ...
    >
      ...
      <el-form-item
        label="文件标签"
        prop="labelId"
        v-if="ruleForm.typeId != ''"
      >
        <div class="label-box">
          <span
            :class="{ 'active-span': ruleForm.labelId == item.id }"
            v-for="(item, key) in fileLableList"
            :key="key"
            @click="handleLabelIdChange(item)"
          >
            {{ item.itemText }}
          </span>
          <i
            class="el-icon-circle-plus-outline add-file-icon"
            @click="addChildrenFlag = true"
          ></i>
        </div>
      </el-form-item>
    ...
    >
      <el-form
        :model="ruleFormTwo"
        :rules="rules"
        ref="ruleFormChildren"
        label-width="160px"
        class="demo-ruleForm"
      >
        <el-form-item label="标签名称" prop="lableName">
          <el-input
            v-model.trim="ruleFormTwo.lableName"
            placeholder="请输入专项行动类型"
            style="width: 70%"
          ></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="addCompanyUser('ruleFormChildren')">
          确 定
        </el-button>
      </span>
    </el-dialog>
    <span slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" @click="register" round>确 定</el-button>
    </span>
  </el-dialog>
</template>

<script>
import { addFileLabel, planFile } from '@/api/accidentManagement'
...
export default {
  ...
  methods: {
    ...
    async getIndustryData() {
      //获取行业分类数据
      let res = await dictItem(true, 40001)
      if (res.code == '0000') {
    ...
    handleSelectionChange() {
      let { sysDictItemEntitys, itemText } = this.fileTyepList.find(
        (v1) => v1.id === this.ruleForm.typeId
      )
