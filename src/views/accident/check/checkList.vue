<template>
  <div id="AcciTemplateList">
    ...
    <div class="content1">
      <div class="stBtn">
        <span class="query-button" @click="goCreate" v-if="datas.find(item => item.key == '120036')">创建检查表</span>
      </div>
      <el-table :data="tableData" style="width: 100%;border:1px solid #ECECEC" :header-cell-style="getRowClass" stripe>
        <el-table-column prop="title" label="模板名称" min-width="250">
          <template slot-scope="scope">
            <el-button @click="goConfig(scope.row.id)" type="text" v-if="datas.find(item => item.key == '120039')">{{ scope.row.title }}</el-button>
            <span v-else>{{ scope.row.title }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="templateType" label="检查表类型" show-overflow-tooltip width="200">
          <template slot-scope="scope">
            <span v-if="scope.row.templateType == 1">专家检查表</span>
            <span v-else>-</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" fixed="right" width="100">
          <template slot-scope="scope">
            <el-button @click="shenhe(scope.row)" type="text" v-if="datas.find(item => item.key == '120037')">编辑</el-button>
            <el-button @click="open(scope.row.id)" type="text" style="color:#000" v-if="datas.find(item => item.key == '120038')">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  data() {
    return {
      ...
      tableData: [],
      typeList: [{ name: "专家检查表", code: 1 }, 
      // { name: "企业检查表", code: 2 }, { name: "行管检查表", code: 4 }
      ],
      datas:[],
    };
  },
  ...
  methods: {
    ...
    shenhe(val) { // 编辑检查表
      ...
      obj.templateType =[val.templateType] // 编辑检查表回显使用
