      ...
      <div class="mt8">
        ...
        >
          ...
          <el-table-column prop="deptName" label="部门" align="center">
            <template slot-scope="scope">
              {{ scope.row.deptNameTop }} / {{ scope.row.deptName }}
            </template>
          </el-table-column>
          <el-table-column
            prop="postName"
...
export default {
  ...
  methods: {
    async initData() {
      this.typeData = this.getStrEnumAll.UserTypeEnum
      await this.initRole()
    ...
    async initRole() {
      ...
      if (res.code === '0000') {
        this.roleData = this.filterRole(res.list, 1)
        console.log(this.roleData, 'this.roleData')
      }
    },
    filterRole(arr, type) {
      let list = []
      let list1 = JSON.parse(JSON.stringify(arr))
      console.log(arr, 'arrrrrrr')
      console.log(list1, 'dfdfdfdfd')
      let headquartersData = window.sessionStorage.getItem('roles').split(',')
      for (let key in headquartersData) {
        if (headquartersData[key] === 'RL1661181862950866945' && type == '1') {
          list = list1.filter(
            (item) => item.roleCode != 'RL1453251619998273537'
          )
        } else if (
          headquartersData[key] === 'RL1661181862950866945' &&
          type == '2'
        ) {
          list = list1.filter(
            (item) =>
              item.roleCode != 'RL1661181862950866945' &&
              item.roleCode != 'RL1453251619998273537'
          )
        }
      }
      return list
    ...
    update(row) {
      ...
      this.$refs.AddAccountRef.branchList = this.branchList
      this.$refs.AddAccountRef.roleList = this.filterRole(this.roleData, 2)
    },
