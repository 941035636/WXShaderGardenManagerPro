<template>
	<div class="policyformmanagement" id="policyformmanagement">
		...
		<div class="content">
			<div class="content1">
				<span class="content1-num">共<span style="color:#333333;">{{totalEquipmentCount}}</span>部</span>，<span class="content1-num">保费总计：<span style="color:#FF4D24;">{{totalPrem}}元</span></span>
				<span class="content1-exports"  @click="exports" v-if="datas.find(item => item.key == '10009')"><img src="../../../static/img/dus.png" alt="">批量导出</span>
			</div>
					...
					<div class="table-data">
						...
						<div class="title6 data-item">
							<div class="state-item">
								<div class="centered-block text-center">
									<p class="font-colo f14 lh30" @click="enquirydetails(row)" v-if="datas.find(item => item.key == '10001')">查看详情</p>
									<p class="font-colo f14 lh30" @click="relation(row)" v-if="datas.find(item => item.key == '10002')">设备清单</p>
									<p class="font-colo f14 lh30" @click="paymentnotice(row)" v-if="row.state == '01' && datas.find(item => item.key == '10003')">支付通知</p>
									<p class="font-colo f14 lh30" @click="premium(row)" v-if="row.state == '01' && datas.find(item => item.key == '10004') && row.pushStatus != '01' && row.pushStatus != '02'">修改保费</p>
									<p class="font-colo f14 lh30" @click="identification(row)" v-if="row.state == '01' && datas.find(item => item.key=='10005')">到账确认</p>
									<p class="font-colo f14 lh30" @click="policyupload(row)" v-if="row.state == '02' && datas.find(item => item.key=='10006')">保单上传</p>
									<p class="font-colo f14 lh30" @click="policyupload(row)" v-if="row.productCodes == '1' && row.state=='08' && datas.find(item => item.key=='10006')">保单上传</p>
...
export default {
	...
	created(){
		this.policyStatu();
		this.getArea();
		if(window.localStorage.getItem('index') == '1' && window.localStorage.getItem('data')){
			this.gettingCache();
		}else{
