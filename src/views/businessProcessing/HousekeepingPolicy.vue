	...
	<div class="policyformmanagement" id="policyformmanagement">
		...
		<div class="content">
			<div class="content1">
				...
				<span class="content1-exports"  @click="exports" v-if="datas.find(item => item.key == '10061')"><img src="../../../static/img/dus.png" alt="">批量导出</span>
			</div>
					...
					<div class="table-data">
						...
						<div class="title6 data-item">
							<div class="state-item">
								<div class="centered-block text-center">
									<p class="font-colo f14 lh30" @click="enquirydetails(row)" v-if="datas.find(item => item.key == '10054')">查看详情</p>
									<p class="font-colo f14 lh30" @click="relation(row)" v-if="datas.find(item => item.key == '10055')">人员清单</p>
									<p class="font-colo f14 lh30" @click="paymentnotice(row)" v-if="row.state == '01' && datas.find(item => item.key == '10056')">支付通知</p>
									<p class="font-colo f14 lh30" @click="premium(row)" v-if="row.state == '01' && datas.find(item => item.key == '10057')">修改保费</p>
									<p class="font-colo f14 lh30" @click="identification(row)" v-if="row.state == '01' && datas.find(item => item.key=='10058')">到账确认</p>
									<p class="font-colo f14 lh30" @click="policyupload(row)" v-if="row.state == '02' && datas.find(item => item.key=='10059')">保单上传</p>
									<p class="font-colo f14 lh30" @click="endorsements(row)" v-if="datas.find(item => item.key == '10060') && row.state == '03' || datas.find(item => item.key == '10060') && row.state == '04' || datas.find(item => item.key == '10007') && row.state == '06'">批单补录</p>
...
export default {
	...
	created(){
		...
		this.getArea();
		this.loadBranch();
		if(window.localStorage.getItem('index') == 'proJZ' && window.localStorage.getItem('data')){
			this.gettingCache();
		}else{
