	...
	<div class="employerPolicy" id="employerPolicy">
		...
		<div class="content">
			<div class="content1">
				...
				<span class="content1-exports"  @click="exports" v-if="datas.find(item => item.key == '10026')"><img src="../../../static/img/dus.png" alt="">批量导出</span>
			</div>
					...
					<div class="table-data">
						<div class="title1 data-item">
							<div class="centered-block">
								...
								<p class="color-gray-3 f14 lh30" v-if="datas.find(item => item.key == '10038')">
								...
								<p class="color-gray-3 f14 lh30" v-if="row.showPerson">
									<span> 人员数量：</span>{{row.insuredsCount}}
								</p>
								<p class="color-gray-3 f14 lh30" v-if="row.showPerson">
									<span> 投保区域：</span>{{row.region}}
								</p>
								<p class="color-gray-3 f14 lh30" v-if="row.policyKind=='01' && row.showPerson">
									<span> 保单邮寄地址：</span><span>{{row.mailAddress}}</span>
								</p>
								<p class="color-gray-3 f14 lh30" v-if="row.policyKind=='00' && row.showPerson">
						...
						<div class="title6 data-item">
							<div class="state-item">
								<div class="centered-block text-center">
									<p class="font-colo f14 lh30" @click="enquirydetails(row)" v-if="datas.find(item => item.key == '10019')">查看详情</p>
									<p class="font-colo f14 lh30" @click="relation(row)" v-if="datas.find(item => item.key == '10020') && row.showPerson">人员清单</p>
									<p class="font-colo f14 lh30" @click="paymentnotice(row)" v-if="row.state == '01' && datas.find(item => item.key == '10021')">支付通知</p>
									<p class="font-colo f14 lh30" @click="premium(row)" v-if="row.state == '01' && datas.find(item => item.key == '10022')">修改保费</p>
									<p class="font-colo f14 lh30" @click="identification(row)" v-if="row.state == '01' && datas.find(item => item.key=='10023')">到账确认</p>
									<p class="font-colo f14 lh30" @click="policyupload(row)" v-if="row.state == '02' && datas.find(item => item.key=='10024')">保单上传</p>
...
<script>
import product from './data/tyProductList.json';
...
export default {
	...
	created(){
		this.policyStatu();
		this.getArea();
		if(window.localStorage.getItem('index') == '3' && window.localStorage.getItem('data')){
			this.gettingCache();
		}else{
	...
	methods: {
			...
			if(res.code == '0000'){
				...
				if(this.total){
					this.data1.forEach(item =>{
						...
						if(item.productName.indexOf('线下录单') != -1) {
							item.showPerson = false;
						} else {
							item.showPerson = true;
						}
					});
				}
		...
		async getLiftStatistics(){
			...
			if(res.code == '0000'){
				...
				this.totalEquipmentCount = this.$stringHelper.formatMoneys(res.data.totalPersonCount);
			}
		},
