	...
	<div class="employerPolicy" id="employerPolicy">
		...
		<div class="content">
			<div class="content1">
				...
				<span class="content1-exports"  @click="exports" v-if="datas.find(item => item.key == '10088')"><img src="../../../static/img/dus.png" alt="">批量导出</span>
			</div>
					...
					<div class="table-data">
						<div class="title1 data-item">
							<div class="centered-block">
								...
								<p class="color-gray-3 f14 lh30" v-if="datas.find(item => item.key == '10089')">
						...
						<div class="title3 data-item">
							<div class="centered-block text-center">
								...
								<p class="font-colo f14 lh30" @click="premium(row)" v-if="row.state == '01' && datas.find(item => item.key == '10083') && row.pushStatus != '01' && row.pushStatus != '02'">修改保费</p>
						...
						<div class="title4 data-item">
							<div class="centered-block">
								...
								<p class="color-gray-3 f14 lh30" v-if="row.invoiceStatus=='02'"><span>发票状态：</span>开票失败</p>
						...
						<div class="title5 data-item">
							<div class="centered-block">
								...
								<p class="color-gray-3 f14 lh30"><span>地址：</span>{{row.location}}</p>
							</div>
						</div>
						<div class="title6 data-item">
							<div class="state-item">
								<div class="centered-block text-center">
									<p class="font-colo f14 lh30" @click="enquirydetails(row)" v-if="datas.find(item => item.key == '10080')">查看详情</p>
									<p class="font-colo f14 lh30" @click="relation(row)" v-if="datas.find(item => item.key == '10081')">人员清单</p>
									<p class="font-colo f14 lh30" @click="paymentnotice(row)" v-if="row.state == '01' && datas.find(item => item.key == '10082')">支付通知</p>
									<p class="font-colo f14 lh30" @click="identification(row)" v-if="row.state == '01' && datas.find(item => item.key=='10084') && row.productCode != '19PR627806'">到账确认</p>
									<p class="font-colo f14 lh30" @click="policyupload(row)" v-if="row.state == '02' && datas.find(item => item.key=='10085')">保单上传</p>
									<p class="font-colo f14 lh30" @click="endorsements(row)" v-if="datas.find(item => item.key == '10086') && row.state == '03' || datas.find(item => item.key == '10086') && row.state == '04'">批单补录</p>
									<p class="font-colo f14 lh30" @click="missMessage(row)" v-if="row.state == '08' && row.productCodes == '1' && datas.find(item => item.key == '10087')">报错报文</p>
									<p class="font-colo f14 lh30" @click="isPuah(row)" v-if="(row.state == '08' || row.state == '02') && row.productCodes == '1' && datas.find(item => item.key == '10040')">出单重推</p>
								</div>
							</div>
						</div>
					</div>
...
export default {
	...
	created(){
		...
		this.getArea();
		this.loadBranch();
		if(window.localStorage.getItem('index') == '8' && window.localStorage.getItem('data')){
			this.gettingCache();
		}else{
	...
	methods: {
		...
		async paymentnotices(){// 发支付通知
			...
			if(res.code == '0000'){
				...
				this.trackEvent('消息','雇主支付通知');
			}
		},
		premium(row){ // 修改保费
			let refs = product.data.find(item=>item.productCode == row.productCode).revisionPrem || 'revisionPremiumDialog' 
