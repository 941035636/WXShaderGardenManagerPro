<template>
	<div class="policyformmanagement" id="policyformmanagement">
		<div class="elesearch">
			...
			<el-row type="flex" class="row-bg">
				...
				<el-col :span="8"><div>
					...
					<el-select v-model="data.branchScope" placeholder="请选择分支机构" class="item2" clearable>
						<el-option v-for="(item,index) in options3" :key="index" :label="item.jtBranchName" :value="item.jtBranchCode"></el-option>
					</el-select>
				</div></el-col>
			</el-row>
			<!-- <el-row type="flex" class="row-bg">
		...
		<div class="content">
			<div class="content1">
				<span class="content1-num">共<span style="color:#333333;">{{totalEquipmentCount}}</span>个</span>，<span class="content1-num">保费总计：<span style="color:#FF4D24;">{{totalPrem}}元</span></span>
				<span class="content1-exports"  @click="exports" v-if="datas.find(item => item.key == '10049')"><img src="../../../static/img/dus.png" alt="">批量导出</span>
			</div>
					...
					<div class="table-data">
						<div class="title1 data-item">
							<div class="centered-block">
								...
								<p class="color-gray-3 f14 lh30" v-if="datas.find(item => item.key == '10050')">
						...
						<div class="title6 data-item">
							<div class="state-item">
								<div class="centered-block text-center">
									<p class="font-colo f14 lh30" @click="enquirydetails(row)" v-if="datas.find(item => item.key == '10043')">查看详情</p>
									<p class="font-colo f14 lh30" @click="relation(row)" v-if="datas.find(item => item.key == '10044')">物业清单</p>
									<p class="font-colo f14 lh30" @click="paymentnotice(row)" v-if="row.state == '01' && datas.find(item => item.key == '10045')">支付通知</p>
									<!-- <p class="font-colo f14 lh30" @click="premium(row)" v-if="row.state == '01' && datas.find(item => item.key == '10046')">修改保费</p> -->
									<p class="font-colo f14 lh30" @click="identification(row)" v-if="row.state == '01' && datas.find(item => item.key=='10047')">到账确认</p>
									<p class="font-colo f14 lh30" @click="policyupload(row)" v-if="row.state == '02' && datas.find(item => item.key=='10048')">保单上传</p>
									<!-- <p class="font-colo f14 lh30" @click="endorsements(row)" v-if="datas.find(item => item.key == '10007') && row.state == '03' || datas.find(item => item.key == '10007') && row.state == '04' || datas.find(item => item.key == '10007') && row.state == '06'">批单补录</p> -->
									<p class="font-colo f14 lh30" @click="missMessage(row)" v-if="row.state == '08' && row.productCodes == '1' && datas.find(item => item.key == '10008')">报错报文</p>
...
export default {
	...
	created(){
		...
		this.getArea();
		this.loadBranch();
		if(window.localStorage.getItem('index') == 'pro1' && window.localStorage.getItem('data')){
			this.gettingCache();
		}else{
	...
	methods: {
			...
			if(res.code == '0000'){
				...
				if(branchCode == 'JT0000'){
					...
					if(branchCode == 'JT0144000') {
						this.options3.push({"jtBranchCode":"JT0144001","jtBranchName":"广东分公司本部"})
					}
		...
		async isPuah(row){ // 出单重推
			onlineProducts.data.forEach(item =>{
