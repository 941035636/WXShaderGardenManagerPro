<template>
	<div class="policy-property mt20">
            ...
            <el-row type="flex" class="row-bg">
                <el-col :span="24">
                    ...
                    <el-table :data="data1" style="width: 100%;margin-top:20px;" highlight-current-row show-header border fit :header-cell-style="getRowClass1">
						<el-table-column label="物业项目名称" min-width="100">
                            <template slot-scope="scope">
                                <p>{{scope.row.name}} <span class="see-style" @click="showPersonDetail(scope.row)">查看</span></p>
                            </template>
                        </el-table-column>
						<el-table-column label="物业具体地址">
                            <template slot-scope="scope">
                                <p>{{scope.row.provinceName}}<span v-if="scope.row.cityName">/{{scope.row.cityName}}</span><span v-if="scope.row.countryName">/{{scope.row.countryName}}</span></p>
                            </template>
                        </el-table-column>
		...
		<el-dialog title="物业信息" :visible.sync="dialogVisible1" width="30%" :before-close="handleClose" class="dialog-p">
			<div class="f14">
				<p v-if="row.name" class="font-black">物业项目名称：{{row.name}}</p>
				...
				<p v-if="row.area">物业管理建筑面积：{{row.area}}（平米）</p>
				<p v-if="row.styleName">项目类型：{{row.styleName}}</p>
			</div>
		</el-dialog>
    </div>
</template>
...
export default {
	...
	methods: {
        ...
        searchview() {
			...
			this.data.pageNum = 1;
			this.currentPage = 1;
			if(!this.regionCoding.length && !this.data.name && !this.data.policyNo && !this.data.endorNo && !this.data.startDateFrom && !this.data.endDateFrom && !this.data.startDateTo && !this.data.endDateTo && !this.data.policyHolder) {
				this.$message({
					message: '请输入至少一个查询条件',
					type: 'warning'
				});
        ...
        async policylistWY(){
			...
			let res  = await this.Elevator.postPropertyList(this.data);
