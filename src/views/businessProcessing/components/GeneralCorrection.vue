						...
						<el-table-column label="冲正后">
							<template slot-scope="scope">
								<el-form-item v-if="scope.row.value == '00'">
									<el-input v-model.trim="scope.row.newCode" placeholder="请输入保单号" :disabled="scope.row.show"></el-input>
								</el-form-item>
								<el-form-item v-if="scope.row.value == '01'">
									<el-date-picker type="date" :picker-options="dealDateStartOptions" placeholder="选择日期" v-model="scope.row.newCode" @change="checkDate" style="width: 100%;" :disabled="scope.row.show" value-format="yyyy-MM-dd"></el-date-picker>
								</el-form-item>
								...
								<el-form-item v-if="scope.row.value == '04'">
									<el-input v-model.trim="scope.row.newCode" placeholder="请输入投保人名称" :disabled="scope.row.show" maxlength="100"></el-input>
								</el-form-item>
								<el-form-item v-if="scope.row.value == '05'">
									<el-input v-model.trim="scope.row.newCode" placeholder="请输入投保人统一信用代码" :disabled="scope.row.show" maxlength="100"></el-input>
								</el-form-item>
							</template>
						</el-table-column>
					</el-table>
...
<script>
import PolicyInfoList from '../../../../static/data/PolicyInfoList.json';
import RequiredCheck from '../js/requiredcheck';
import Reason from './Reason'
export default {
	name:'GeneralCorrection',
	props:{
		'form':Object
	},
	data() {
	...
	methods: {
		getData(){
			this.forms.list = JSON.parse(JSON.stringify(PolicyInfoList.data));
			this.forms.list.forEach(item =>{
				if(item.value == '00'){
					...
					item.code = this.form.policyHolder
				}else if(item.value == '05'){
					item.code = this.form.threeInOneCode
				}
			})
			this.$nextTick(()=>{
                this.getDatas();
...
        },
		...
		handleSelectionChange(val) {
			...
			let list = this.$stringHelper.getArrDifference(this.forms.list,val);
			this.forms.list.forEach(item1 =>{
				list.forEach(item =>{
					if(item.value == item1.value){
						item1.newCode = '';
