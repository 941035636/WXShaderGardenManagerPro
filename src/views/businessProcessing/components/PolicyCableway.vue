            ...
            <el-row type="flex" class="row-bg">
                <el-col :span="24">
                    ...
                    <el-table :data="data1" style="width: 100%;margin-top:20px;" highlight-current-row show-header border fit :header-cell-style="getRowClass1">
						<el-table-column label="索道名称" min-width="100">
                            <template slot-scope="scope">
                                <p>{{scope.row.equipmentsName}} 
                                </p>
                            </template>
                        </el-table-column>
						<el-table-column label="设备代码/类型">
                            <template slot-scope="scope">
                                <p>{{scope.row.equipmentsCode}} 
                                <span class="see-style" @click="showPersonDetail(scope.row)">查看</span>
                                </p>
                                <p>索道类型：{{scope.row.equipmentsTypeName}}</p>
                                <p>上年度运送乘客人次(万)：{{scope.row.passengerLastyearName}}</p>
                            </template>
                        </el-table-column>
...
export default {
	...
	methods: {
        ...
        searchview() {
			...
			this.data.pageNum = 1;
			this.currentPage = 1;
			if(!this.regionCoding.length && !this.data.equipmentsCode && !this.data.equipmentsName&& !this.data.name && !this.data.policyNo && !this.data.endorNo && !this.data.startDateFrom && !this.data.endDateFrom && !this.data.startDateTo && !this.data.endDateTo && !this.data.policyHolder) {
				this.$message({
					message: '请输入至少一个查询条件',
					type: 'warning'
				});
        ...
        async policylistWY(){
			...
			let res  = await this.Elevator.getPropertyList(this.data);
		...
		async exports(){
			...
			this.ResourcesService.exportsViewFile(this.$apiUrl.CABLEWAYEXPORT,data)
		},
	}
};
</script>
