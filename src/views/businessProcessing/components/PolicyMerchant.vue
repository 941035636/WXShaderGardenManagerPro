<template>
	<div class="PolicyMerchant mt20">
        <div class="elesearch">
			<el-row type="flex" class="row-bg">
                 <el-col :span="8">
					<div>
						<span class="item3">商户地址：</span>
                 ...
                 <el-col :span="8">
					<div>
						<span class="item3">详细地址：</span>
						<el-input size="medium" v-model.trim="data.address" placeholder="请输入" class="item2" clearable ></el-input>
					</div>
				</el-col>
				<el-col :span="8">
					<div>
            ...
            <el-row type="flex" class="row-bg">
                <el-col :span="24">
					<div style="display:flex;justify-content:space-between">
						...
						<div>
                            <span class="mr10">查询结果：共 {{total}} 条记录 </span>
                    ...
                    <el-table :data="data1" style="width: 100%;margin-top:20px;" highlight-current-row show-header border fit :header-cell-style="getRowClass1">
						<el-table-column label="商户地址" min-width="100">
                            <template slot-scope="scope">
                                <p>{{scope.row.areaNmae}} 
                                </p>
                            </template>
                        </el-table-column>
		...
		<el-dialog title="设备信息" :visible.sync="dialogVisible1" width="30%" :before-close="handleClose" class="dialog-p">
			<div class="f14">
				<p v-if="row.equipmentsName" class="font-black">索道名称：{{row.equipmentsName}}</p>
				<p v-if="row.equipmentsCode">设备代码：{{row.equipmentsCode}}</p>
				<p v-if="row.equipmentsTypeName">索道类型：{{row.equipmentsTypeName}}</p>
				<p v-if="row.passengerLastyearName">上年度运送乘客人次(万)：{{row.passengerLastyearName}}</p>
			</div>
		</el-dialog>
    </div>
</template>
...
export default {
	...
	methods: {
        ...
        searchview() {
			...
			this.data.pageNum = 1;
			this.currentPage = 1;
			if(!this.regionCoding.length && !this.data.address && !this.data.equipmentsName&& !this.data.name && !this.data.policyNo && !this.data.endorNo && !this.data.startDateFrom && !this.data.endDateFrom && !this.data.startDateTo && !this.data.endDateTo && !this.data.policyHolder) {
				this.$message({
					message: '请输入至少一个查询条件',
					type: 'warning'
				});
        ...
        async policylistWY(){
			...
			let res  = await this.Elevator.getShopList(this.data);
			if(res.code == '0000' && res.list.length != 0) {
				this.total = res.total;
				res.list.forEach(i => {
                   i.areaNmae =  [i.provinceName,i.cityName,i.countryName].filter(item=>item).join('/')
				})
				this.data1 = res.list;
			} else {
				this.total = 0;
		...
		async exports(){
			...
			this.ResourcesService.exportsViewFile(this.$apiUrl.SHOPEXPORT,data)
		},
	}
};
</script>
