	...
	<div id="batchManagement" class="batch-management">
		<div class="elesearch">
			<el-row type="flex" class="row-bg">
				<el-col :span="8">
					<div style="display:flex;">
						...
						<el-input v-model.trim="formData.idNo" placeholder="请输入证件号码" size="small" clearable class="item2"></el-input>
					</div>
				</el-col>
				<el-col :span="8">
					<div style="display:flex;">
			...
			<el-row type="flex" class="row-bg">
				<el-col :span="8">
					<div style="display:flex;">
						<span class="item3">人员名称：</span>
						<el-input v-model.trim="formData.name" placeholder="请输入人员名称" size="small" clearable class="item2"></el-input>
					</div>
				</el-col>
				<el-col :span="8">
					<div style="display:flex;">
				...
				<el-col :span="8">
					<div style="display:flex;">
						<span class="item3">人员批改类型：</span>
						<el-select v-model="formData.endorType" placeholder="请选择人员批改类型" size="small" clearable>
					...
					<el-row type="flex" class="row-bg">
						<el-col :span="9">
							<div style="padding:15px 10px;box-sizing: border-box;">
								<p><span class="col-blue">{{item.name}}</span></p>
								<p><span class="col-gre">{{item.idTypeName}}</span></p>
								<p><span>证件号码：{{item.idNo}}</span></p>
								<p><span>人员批改类型：<span>{{item.endorType == 1 ? '批增' : '批减'}}</span></span></p>
							</div>
						</el-col>
						<el-col :span="10">
							<div style="padding:15px 10px;box-sizing: border-box;">
...
import pdPerJson from '../../../static/data/pdPerType.json'
export default {
	data() {
		return {
	...
	created(){
		this.types();
		if(window.localStorage.getItem('index') == '5-2'){
			this.gettingCache();
		}else{
	...
	methods: {
		...
		async getPersonList(){
			...
			let res = await this.Elevator.getPersonDataList(this.formData);
			if(res.code == '0000' && res.total != 0){
				res.list.forEach(item => {
					...
					item.idTypeName = pdPerJson.idType.find(subItem => subItem.value == item.idType) ? pdPerJson.idType.find(subItem => subItem.value == item.idType).label : ''
				})
				this.data1 = res.list;
