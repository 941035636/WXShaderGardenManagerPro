<template>
	<div class="employerPolicy" id="employerPolicy">
		<div class="elesearch">
			<el-row type="flex" class="row-bg">
				...
				<!-- <el-col :span="8"><div>
					...
					<el-select v-model="data.productCode" placeholder="请选择" class="item2" clearable>
				...
				<el-col :span="8"><div>
					...
					<el-select v-model="data.branchScope" placeholder="请选择分支机构" class="item2" clearable>
						<el-option v-for="(item,index) in options9" :key="index" :label="item.jtBranchName" :value="item.jtBranchCode"></el-option>
					</el-select>
				</div></el-col>
			</el-row>
			<el-row type="flex" class="row-bg">
			...
			<el-row type="flex" class="row-bg">
				<el-col :span="8"><div>
					...
					<el-select v-model="data.payType" placeholder="请选择支付方式" class="item2" clearable>
						<el-option v-for="item in options6" :key="item.value" :label="item.label" :value="item.value"></el-option>
					</el-select>
				</div></el-col>
				<el-col :span="8"><div>
					<span class="item3">下单渠道</span>
					<el-select v-model="data.source" placeholder="请选择下单渠道" class="item2" clearable>
		...
		<div class="content">
			<div class="content1">
				<!-- <span class="content1-num">共<span style="color:#333333;">{{totalEquipmentCount}}</span>人</span>，<span class="content1-num">保费总计：<span style="color:#FF4D24;">{{totalPrem}}元</span></span> -->
				<span class="content1-exports"  @click="exports" v-if="datas.find(item => item.key == '10070')"><img src="../../../static/img/dus.png" alt="">批量导出</span>
			</div>
					...
					<div class="table-data">
						<div class="title1 data-item">
							<div class="centered-block">
								...
								<p class="color-gray-3 f14 lh30" v-if="datas.find(item => item.key == '10071')">
								...
								<p class="color-gray-3 f14 lh30" v-if="row.branchName">
									<span> 分支机构：</span>{{row.branchName}}
								</p>
								...
								<p class="color-gray-3 f14 lh30" v-if="row.policyKind=='00'">
									<span> 电子邮箱：</span><span>{{row.policyMailAddress}}</span>
								</p>
							</div>
						</div>
						...
						<div class="title3 data-item">
							<div class="centered-block text-center">
								<p class="font-colo1 f14 lh30" v-if="row.productCodes == '1' && row.state=='08'"><span></span>{{row.appStatus}}<el-button type="text" icon="el-icon-question" @click="reasonsFailure(row)"></el-button></p>
								<p class="font-colo1 f14 lh30" v-else><span></span>{{row.appStatus}}<span v-if="row.pushStatus"> / {{row.pushStatus == '00' ? '未推送' : row.pushStatus == '01' ? '已推送' : row.pushStatus == '02' ? '推送失败' : ''}}</span></p>
								...
								<p class="font-colo f14 lh30" @click="premium(row)" v-if="row.state == '01' && datas.find(item => item.key == '10065') && row.pushStatus != '01' && row.pushStatus != '02'">修改保费</p>
								<p class="color-gray-3 f14 lh30">{{row.payTypeName}}</p>
							</div>
						</div>
						...
						<div class="title6 data-item">
							<div class="state-item">
								<div class="centered-block text-center">
									<p class="font-colo f14 lh30" @click="enquirydetails(row)" v-if="datas.find(item => item.key == '10063')">查看详情</p>
									<p class="font-colo f14 lh30" @click="relation(row)" v-if="datas.find(item => item.key == '10073')">商铺地址</p>
									<p class="font-colo f14 lh30" @click="relationPerson(row)" v-if="row.showPerBtn">人员清单</p>
									<p class="font-colo f14 lh30" @click="paymentnotice(row)" v-if="row.state == '01' && datas.find(item => item.key == '10064')">支付通知</p>
									<p class="font-colo f14 lh30" @click="identification(row)" v-if="row.state == '01' && datas.find(item => item.key=='10066') && row.productCode != '19PR627806'">到账确认</p>
									<p class="font-colo f14 lh30" @click="policyupload(row)" v-if="row.state == '02' && datas.find(item => item.key=='10067')">保单上传</p>
									<p class="font-colo f14 lh30" @click="endorsements(row)" v-if="datas.find(item => item.key == '10068') && row.state == '03' || datas.find(item => item.key == '10068') && row.state == '04'">批单补录</p>
									<p class="font-colo f14 lh30" @click="missMessage(row)" v-if="row.state == '08' && row.productCodes == '1' && datas.find(item => item.key == '10069')">报错报文</p>
									<p class="font-colo f14 lh30" @click="isPuah(row)" v-if="(row.state == '08' || row.state == '02') && row.productCodes == '1' && datas.find(item => item.key == '10072')">出单重推</p>
								</div>
							</div>
						</div>
					</div>
		...
		<el-dialog title="报错报文" :visible.sync="dialogVisible" width="40%">
			<span>{{data2}}</span>
			<span slot="footer" class="dialog-footer">
				<el-button type="primary" @click="dialogVisible = false">确 定</el-button>
			</span>
		</el-dialog>
		<el-dialog title="商户地址" :visible.sync="dialogVisibleAddress" width="40%">
		...
		<!-- 修改保费Dialog -->
		<revisionPremiumDialog ref="revisionPremiumDialog" @successCallback="searchview"></revisionPremiumDialog>
	...
	methods: {
		policyStatu(){ //根据权限获取配置文件
			...
			if(window.sessionStorage.getItem('branchCode') == 'JT0000'){
				...
				for(var i=0;i<options3.length;i++){
					if(window.sessionStorage.getItem('branchCode') == options3[i].branchCode || options3[i].branchCode == 'JT0000'){
			...
			this.options6 = placeSinglechannel.data;
		},
		async loadBranch(){
			...
			if(res.code == '0000'){
				...
				if(branchCode == 'JT0000'){
					...
					this.options9 = res.list;
				}else{
					if(branchCode == 'JT0144000') {
						this.options9.push({"jtBranchCode":"JT0144001","jtBranchName":"广东分公司本部"})
					}
					res.list.forEach(item =>{
						if(item.jtBranchCode == branchCode){
							this.options9.push(item)
						}
					})
				}
            }
		...
		async policylist(){ // 获取投保单列表
			...
			if(res.code == '0000'){
				...
				if(this.total){
					this.data1.forEach(async item =>{
						let dataSearch = {
							applicationFormCode:item.appFormCode,
							pn:1,
							ps:10
						}
						let resBD = await this.UserService.getInsurance(dataSearch);
						if(resBD.code == '0000' && resBD.list.length != 0) {
							if(resBD.list.find(subItem => subItem.objectType == '22')) {
								item.showPerBtn = true;
							} else {
								item.showPerBtn = false;
							}
						}
						...
						}else if(item.payType=='01' && item.payChannel == '06'){	// 微保-支付宝
		...
		async reasonsFailure(row){//出单失败提示
		...
		async paymentnotices(){// 发支付通知
			...
			if(res.code == '0000'){
				...
				this.trackEvent('消息','商户支付通知');
			}
		},
		premium(row){ // 修改保费
			let refs =  'revisionPremiumDialog' 
			this.$refs[refs].getappdetails(row.appFormCode)
		},
		...
		exports(){ // 投保单导出
			if(this.total <= 5000){
				let href = this.$apiUrl.EXPORTSS + '?userCode=' + window.sessionStorage.getItem('userCode') + '&holderName=' + this.data.holderName + '&appCode=' + this.data.appCode + '&policyNo=' + this.data.policyNo + '&orderCode=' + this.data.orderCode + '&productCode=' + this.data.productCode + '&provinceCode=' + this.data.provinceCode + '&cityCode=' + this.data.cityCode + '&countryCode=' + this.data.countryCode + '&state=' + this.data.state + '&appStartFrom=' + this.data.appStartFrom + '&appStartEnd=' + this.data.appStartEnd + '&issueDateStart=' + this.data.issueDateStart + '&issueDateEnd=' +  this.data.issueDateEnd + '&polStartFrom=' +this.data.polStartFrom +  '&polStartEnd=' +this.data.polStartEnd   + '&financeStatus=' + this.data.financeStatus + '&payType=' + this.data.payType + "&productCategory1=" + this.data.productCategory1 + "&source=" + this.data.source + "&branchScope=" + this.data.branchScope;
				this.$logger.info(
					'M1-004','',
					{
