...
import { addorganizationfile } from '@/api/userDataManagement'
export default {
  // props: {
  //   fileImportShow:{
  //     type: Boolean,
  ...
  methods: {
    ...
    downloadTemplate() {
      ...
      a.href = '/static/file/政府平台-贵州省安责险应保企业导入模版.xlsx'
      a.download = '政府平台-贵州省安责险应保企业导入模版.xlsx'
      a.click()
    },
    // 确认上传
    async confirmImportFile() {
      ...
      try {
        let { code } = await addorganizationfile(form)
        if (code == '0000') {
