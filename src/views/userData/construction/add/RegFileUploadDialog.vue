...
<script>
import { convertFileToEntity } from '@/api/Construction.js'
export default {
  // props: {
  //   fileImportShow:{
  //     type: Boolean,
  ...
  methods: {
    ...
    downloadTemplate() {
      ...
      a.href = '/static/file/政府平台-建筑施工项目-导入模板.xlsx'
      a.download = '政府平台-建筑施工项目-导入模板.xlsx'
      a.click()
    },
    // 确认上传
    async confirmImportFile() {
      ...
      try {
        let { code } = await convertFileToEntity(form)
        if (code == '0000') {
          ...
          this.$parent.mationsList()//导入成功后提交，调用父页面列表方法
        } else {
          this.isButLoading = false
        }
      } catch (error) {
