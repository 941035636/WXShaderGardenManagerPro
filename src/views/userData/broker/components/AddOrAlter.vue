    ...
    >
      ...
      <div v-if="ruleForm.orgType != 1">
        ...
        <el-form-item label="总部编码">
          <el-input
            v-model="ruleForm.headquartersCode"
            class="w250"
            placeholder="请输入总部编码"
          ></el-input>
        </el-form-item>
      ...
      <div class="title">
        <span></span>
        <span class="color-blue f16 ml10 mr20">安责险业务服务人姓名</span>
      </div>
      <el-row>
        <el-col :span="10">
...
import { getStreet } from '@/api/Areas'
...
export default {
  ...
  methods: {
    ...
    async getDict() {
      // 获取字典表行业和机构类型、服务项目
      let { OrganizationTypeEnum, ServiceItemsEnum } = this.getStrEnumAll
      this.typeData = OrganizationTypeEnum
      this.serviceItems = ServiceItemsEnum
      let res = await getDictData(true, '20003')
      if (res.code == '0000') this.industryData = res.data
    },
    async initData() {
      if (!this.id) return
      let data = {
      ...
      if (res.code === '0000') {
        if (res.data.headquartersName) this.totalCom = true
    ...
    async getAllArea() {
      ...
      if (res.code == '0000') this.allArea = res.data
    },
    clear() {
      this.$nextTick(() => {
    ...
    async configRegister() {
      ...
      let res = ''
      if (this.id) {
        ;(this.ruleForm.servicename = 'insuranceBrokerSvc'), (res = await putBranch(this.ruleForm))
      } else {
        res = await addInsuranceBranch(this.ruleForm)
      }
      if (res.code === '0000') this.$baseMessage(this.id ? '修改成功' : '添加成功', 'success')
      this.clear()
    },
    register() {
      let text = '确定要新增保险经纪机构基本信息？'
      if (this.id) text = '确定要修改保险经纪机构基本信息？'
    ...
    async handleChangeArea(e) {
      ...
      if (e[3]) {
        ...
        if (res.code == '0000') this.ruleForm.streetName = res.data[e[3]]
      }
    },
  },
}
