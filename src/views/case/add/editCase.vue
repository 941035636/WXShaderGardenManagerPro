...
export default {
  ...
  methods: {
    async init() {
      ...
      if (res.code === '0000') {
        this.getPolicy(res.data.policyRelationResp)
        let data = res.data.caseInfoEntityResp
        if (
          res.data.caseSiteSurveyResps &&
          res.data.caseSiteSurveyResps.length
        ) {
          this.$refs.report.hasSite = true
        }
        data.caseSiteSurveyParams = res.data.caseSiteSurveyResps
        data.caseSiteSurveyParams = res.data.caseSiteSurveyResps.map((item) => {
          if (item.files) {
            item.fileRecordIds = item.files.map((i) => {
              return i.id
            })
            item.fileRecord = item.files
          } else {
            item.fileRecordIds = []
            item.fileRecord = []
          }
        ...
        if (
          ...
          res.data.casePaymentRecordResps.length
        ) {
          this.$refs.report.hasPay = true
        }
        data.casePaymentRecordParams = res.data.casePaymentRecordResps.map(
          (item) => {
            if (item.files) {
              item.fileRecordIds = item.files.map((i) => {
                return i.id
        ...
        data.specAccidentTypeIds =
          res.data.caseInfoEntityResp.specAccidentItems.map((item) => {
            return item.itemValue
          })
        data.areaCascaser = [
          res.data.caseInfoEntityResp.accidentProvinceCode,
          res.data.caseInfoEntityResp.accidentCityCode,
          res.data.caseInfoEntityResp.accidentCountyCode,
        ]
        if (
          res.data.caseInfoEntityResp.files &&
          res.data.caseInfoEntityResp.files.length
        ) {
          data.accidentReportIds = res.data.caseInfoEntityResp.files.map(
            (item) => {
              return item.id
            }
          )
        ...
        this.$nextTick(() => {
          this.$refs.report.files = res.data.caseInfoEntityResp.files
          this.$refs.report.form = data
        })
      }
    },
    openInsured() {
    ...
    async submit() {
      ...
      console.log(this.caseInfoEntity, '0000000')
