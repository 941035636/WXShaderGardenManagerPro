<template>
<div>
    <el-dialog title="设备信息" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
        <el-form :model="form" :rules="rules" ref="ruleForm" class="demo-ruleForm">
            ...
            <el-form-item v-if="form.radio=='2' ||  eleTemplate=='4'" label="出厂编号" :label-width="formLabelWidth" prop="licenceNo">
                <el-input class="ts-ele-input" v-model.trim="form.licenceNo" autocomplete="off" placeholder="出厂编号(必填)" @change="detectionChanges"></el-input>
            </el-form-item>
            ...
            <el-form-item label="电梯坐落地址" :label-width="formLabelWidth" prop="selectedOptions">
					<el-cascader v-if="options.lenght != 0" expand-trigger="hover" :options="options" v-model="form.selectedOptions"  @change="handleChange" style="width: 100x">
					</el-cascader>
            </el-form-item>
            ...
            <el-form-item label="电梯型号"  v-if="eleTemplate =='4'" :label-width="formLabelWidth" prop="elevatorModel">
...
export default {
    data() {
        var equipmentsNumber = (rule, value, callback) => {
    		const equipmentsCodeReg = /^[a-zA-Z0-9]{18,25}$/
    		if (!value) {
    		...
    		setTimeout(() => {
                if (equipmentsCodeReg.test(value)) {
          			...
          			callback(new Error('设备编号为18到25位的数字和字母'))
        		}
    		}, 100)
          };
        ...
        return {
            ...
            rules: {
                    equipmentsCode: [
                        { required: true,validator: equipmentsNumber,trigger: 'change' },
                    ],
                    licenceNo: [
                    ...
                    elevatorYear: [
                        ...
                        { min: 0, max: 99, type: 'number', required: true, message: '请输入正确使用年限（0~99）', trigger: 'change' }
                    ],
                    useOfUnit: [
    ...
    methods: {
        ...
        flagTrue(productCode) {
            
            this.productDisplay(productCode); // 获取产品元素，以及对应的因子
            this.getArea()
            let status = this.ElevatorNorm.template(productCode);   // 模版
            this.eleTemplate= status                        
        },
        // 获取地区
        async getArea(){
        ...
        async productDisplay(productCode) {
            ...
            if (code == "0000") {
                ...
                if( this.form.productCode ==this.$globalProductCode.PRODUCT_CODE_NJ && productClassifyList[0].code=="19FA149436"){
                    this.elevatorSite.list = [];
                ...
                if (valid) {
                  let status =  this.ElevatorNorm.area(this.form);
                    if(status=='0000'){
                        ...
                        this.form.userCode = window.localStorage.getItem("userCode"); // 用户code
                        const params = await this.productService.elementAdd(this.form);
                        let {code,data} = params;
                        if (code == "0000") {
                            let elevatorMessage  = JSON.parse(window.sessionStorage.getItem('elevatorMessage'))
                            elevatorMessage.push(data)  
