		...
		<div class="content-table">
			...
			<el-table ref="multipleTable" :data="dataCaseList" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
				...
				<el-table-column label="报案人">
					<template slot-scope="scope" v-if="scope.row.reportName">{{scope.row.reportName}}（{{scope.row.reportPhone}}）</template>
				</el-table-column>
				<el-table-column prop="reportTime" label="报案时间"></el-table-column>
				<el-table-column label="状态">
					<template slot-scope="scope"><span style="color:#fa8c16">{{scope.row.reportStatusName}}</span><span v-if="scope.row.substate == '1'" style="color:#f5222d">  (中止)</span></template>
				</el-table-column>
				<el-table-column prop="branchName" label="所属分支机构"></el-table-column>
				<el-table-column label="服务人员">
					<template slot-scope="scope" v-if="scope.row.servicerName">{{scope.row.servicerName}}<span v-if="scope.row.servicerPhone">（{{scope.row.servicerPhone}}）</span></template>
				</el-table-column>
			</el-table>
		</div>
		...
		<el-dialog title="转移服务人员" :visible.sync="dialogVisible" width="50%" :before-close="resetData">
			<div class="bor-bot mb20">
				...
				<el-row class="mb20 mt20 f14">
					<el-col :span="8">
						<span class="text-left">服务人员:</span>
						<span>{{showData.servicerName}}</span>
					</el-col>
					<el-col :span="8">
						<span class="text-left">电话号码:</span>
						<span>{{showData.servicerPhone}}</span>
					</el-col>
					<el-col :span="8">
						<span class="text-left">所属分公司:</span>
						<span>{{showData.branchName}}</span>
					</el-col>
				</el-row>
				<el-row class="mb20">
					<el-col :span="8">
...
export default {
	...
	methods: {
		...
		async caseList(flag){ 
			if(!flag) {
				if(this.serviceCode.length != 2 && !this.formInline.reportNo) {
			...
			if(this.serviceCode.length){
				this.formInline.servicerCode = this.serviceCode[1];
			} else {
				this.formInline.servicerCode = '';
			}
			...
			if(res.code == '0000'){
				this.dataCaseList = res.list.map(item=>{
					...
					let branchName = this.branchList.find(v2=>v2.jtBranchCode == item.branchCode) ? this.branchList.find(v2=>v2.jtBranchCode == item.branchCode) : this.branchList.find(v2=>v2.jtBranchName == item.branchCode) ? this.branchList.find(v2=>v2.jtBranchName == item.branchCode) : null
		...
		async choosePro(row,index,flag) {
			if(!flag) {
				...
				let obj = {
					...
					desc:true,
					reportNo:null,
					servicerCode:row.userCode,
					pn:1,
					ps:10000,
				};
				let res = await this.caseService.getCaseList(obj);
				if(res.code == '0000'){
					if(res.total) {
		...
		async submitForm() {
			...
			let res = await this.caseService.transforServicor(this.someParams);
			if(res.code == '0000') {
		...
		async submitAll() {
			...
			let res = await this.caseService.transforServicorAll(this.someParams);
			if(res.code == '0000') {
