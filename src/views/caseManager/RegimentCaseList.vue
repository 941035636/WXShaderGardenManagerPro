<template>
	<div id="RegimentCaseList">
		...
		<div v-if="istFlag" class="no-data">
			暂无数据
		</div>
		...
		<div class="content-table" v-else>
			<div class="content-up">
				...
				<el-button v-if="dataJurisdiction.find(item => item.key == '40005')" class="content-up-btn" type="primary" @click="exports" round>导出</el-button>
			</div>
			<div class="content">
		...
		<div class="content-total" v-if="!istFlag">
...
import product from '../businessProcessing/data/tyProductList.json';
...
export default {
	...
	methods: {
		...
		async exports(){ // 投保单导出
			if(this.total<=500){
				let href = this.$apiUrl.CASE_DCTY + '?riskType=' + this.formInline.riskType + (this.formInline.reportNo?('&reportNo=' + this.formInline.reportNo):'') + (this.formInline.policyNo?('&policyNo=' + this.formInline.policyNo):'') + (this.formInline.identityNumber?('&identityNumber=' + this.formInline.identityNumber):'') + (this.formInline.riskProvinceCode?('&riskProvinceCode=' + this.formInline.riskProvinceCode):'') + (this.formInline.riskCityCode?('&riskCityCode=' + this.formInline.riskCityCode):'') + (this.formInline.riskAreaCode?('&riskAreaCode=' + this.formInline.riskAreaCode):'') + (this.formInline.reportTimeStart?('&reportTimeStart=' + this.formInline.reportTimeStart):'') + (this.formInline.reportTimeEnd?('&reportTimeEnd=' + this.formInline.reportTimeEnd):'') + (this.formInline.channelStatus?('&channelStatus=' + this.formInline.channelStatus):'') + (this.formInline.branchCode?('&dataBranchCode=' + this.formInline.branchCode):'') + (this.formInline.productCode?('&productCode=' + this.formInline.productCode):'');
				this.ResourcesService.exportsViewFile(href)
				this.trackEvent('统计','导出团意案件');
			} else {
				this.$message({
