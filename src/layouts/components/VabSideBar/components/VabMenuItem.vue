  ...
  export default {
    ...
    methods: {
      ...
      handleLink(e) {
        ...
        // 如果jump 是true 跳转 所以判断加个非
        const isJump = this.routeChildren.meta.isJump
        console.log(isJump, this.routeChildren, '-----')
        // if (!isJump && routePath !== 'index') {
        //   this.$baseMessage('当前暂未开发，敬请期待！', 'warning')
        //   return false
        // }
        console.log(routePath + '777', 1, this.routeChildren)
        if (target === '_blank') {
          if (isExternal(routePath)) {
