export class address {
  ...
  smart(event) {
    ...
    copyaddress.forEach((res, index) => {
      if (res) {
        ...
        detailAddresss = detailAddresss + addressObj.location
        obj = Object.assign(obj, addressObj)
        obj.location = detailAddresss
  ...
  smatrAddress(event) {
    ...
    if (address.indexOf('gd') != -1) {
      address = address.replace('gd', '广东省')
    }
    let matchAddress = ''
              ...
              if (res['label'].indexOf(matchAddress) != -1) {
                ...
                if (smartObj.province) {
                  if (!smartObj.city) {
                    ...
                    if (
                      res.label == matchAddress ||
                      res.label.replace(/(区|市|县)$/g, '') == matchAddress
                    ) {
                      matchCounty.push({
                        county: res.label,
                        countyCode: res.value,
                    ...
                    res.value.slice(0, 4) == smartObj.cityCode.slice(0, 4)
                  ) {
                    matchCounty.push({
                      county: res.label,
                      countyCode: res.value,
    ...
    for (let endIndex = 0; endIndex < address.length; endIndex++) {
      ...
      this.addressList.forEach((el) => {
        if (el.value == smartObj.provinceCode) {
          if (
            smartObj.province == '北京市' ||
            smartObj.province == '天津市' ||
            smartObj.province == '上海市' ||
            ...
            el.children.forEach((element) => {
              if (element.value == smartObj.cityCode) {
                element.children.forEach((item) => {
                  if (item.value == smartObj.countyCode) {
                    if (item.children) {
                      item.children.forEach((res) => {
                        if (res['label'].indexOf(matchAddress) != -1) {
                          matchStreet.push({
